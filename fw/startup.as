	...
	; Compiler options:
	;
	; -oHitSensor.cof -mHitSensor.map --summary=default --output=default \
	; usart.p1 HitSensor.p1 --chip=16F88 -P --runtime=default --opt=default \
...
;	20 bytes of RAM objects in bank 1 to zero

	psect	init
	global	__Lrbss_1
	movlw	low(__Lrbss_1)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_1+20)		;load the high address+1 into W
	fcall	clear_ram		;go and zero that range of addresses

;	90 bytes of RAM objects in bank 2 to zero

	psect	init
	bsf	STATUS,7		;select indirect bank 2/3
	global	__Lrbss_2
	movlw	low(__Lrbss_2)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_2+90)		;load the high address+1 into W
	fcall	clear_ram		;go and zero that range of addresses
